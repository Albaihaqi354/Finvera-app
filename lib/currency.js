 ...
 * Handles both IDR format (1.500.000) and USD/EUR format (1,234.56).
 * @param {string} str
 * @param {string} currencyCode - used to determine the decimal separator
 */
export function parseFormattedNumber(str, currencyCode = 'IDR') {
  if (!str) return 0
  const cur = getCurrency(currencyCode)

  let cleaned = String(str)
  if (cur.code === 'IDR' || cur.code === 'JPY') {
    // IDR/JPY use dot as thousands separator — strip dots only
    cleaned = cleaned.replace(/\./g, '').replace(/[^0-9\-]/g, '')
  } else {
    // Other currencies: comma = thousands separator, dot = decimal separator
    cleaned = cleaned.replace(/,/g, '').replace(/[^0-9.\-]/g, '')
  }
 ...
 * @param {string} fromCurrency - The original currency of the amount
 * @param {string} toCurrency   - The target display currency code
 * @param {object} rates        - Map of { currencyCode: rateFromIDR }
 * @returns {number}
 */
export function convertAmount(amount, fromCurrency = 'IDR', toCurrency = 'IDR', rates = null) {
  if (!rates || fromCurrency === toCurrency) return parseFloat(amount) || 0
  ...
  // Convert to base (IDR) first, then to target currency
  const baseAmount = (parseFloat(amount) || 0) / fromRate
  return baseAmount * toRate
}

/**
 * Format an amount from its original currency into the target display currency,
 * applying conversion if rates are available and currencies differ.
 *
 * @param {number} amount          - Raw amount
 * @param {string} itemCurrency    - Original currency of the item (e.g. account's currency)
 * @param {string} displayCurrency - Target currency code for viewing
 * @param {object|null} rates      - Exchange rates map
 */
export function formatConverted(amount, itemCurrency = 'IDR', displayCurrency = 'IDR', rates = null) {
  const converted = rates
    ? convertAmount(amount, itemCurrency, displayCurrency, rates)
    : parseFloat(amount) || 0
...
export async function fetchExchangeRates() {
  try {
    // Call our own Next.js server-side proxy instead of Frankfurter directly.
    // This avoids browser CORS restrictions — the server fetches on our behalf.
