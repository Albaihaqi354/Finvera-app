...
import { PlusCircle, Search, ChevronDown, X, Trash2, ChevronLeft, ChevronRight } from 'lucide-react'
...
function TransactionsContent() {
  ...
  const handleAddSubmit = (e) => {
    ...
    if (newTx.type === 'transfer' && !newTx.targetAccountId) return
    if (newTx.type !== 'transfer' && !newTx.categoryId) return
    ...
    addTransaction({
      ...
      categoryId: newTx.type === 'transfer' ? (categories.find(c => c.type === 'transfer')?.id || newTx.categoryId) : newTx.categoryId,
      ...
      <div className="grow flex flex-col min-w-0 border-l border-brand-black/5 pl-6">
        ...
        <div className="flex items-center justify-between bg-[#F8F8F8] rounded-2xl px-5 py-3 mb-4">
          ...
          <div className="flex items-center gap-4">
            <span className="text-xs font-bold text-brand-black/50">
              Total Income <span className="text-[#F14C4C]">$ {totalIncome.toFixed(2)}</span>
            </span>
            <span className="text-xs font-bold text-brand-black/50">
              Total Expense <span className="text-[#009E9E]">$ {totalExpense.toFixed(2)}</span>
            </span>
          </div>
        </div>
        ...
        {activeSubTab === 'Transaction List' ? (
          <div className="bg-white rounded-2xl border border-brand-black/5 flex-1 flex flex-col min-h-0 overflow-hidden">
            <div className="grid grid-cols-[100px_1fr_120px_160px_1fr_40px] gap-2 px-5 py-3 border-b bg-[#F8F8F8]">
                ...
                <div key={group.date}>
                  ...
                  {group.items.map(tx => {
                    ...
                    return (
                      <div
                        key={tx.id}
                        className="grid grid-cols-[100px_1fr_120px_160px_1fr_40px] gap-2 px-5 py-3.5 border-b hover:bg-[#F8F8F8] items-center group"
                      >
                ...
                {selectedDayTx.length === 0 ? (
                  ...
                  selectedDayTx.map(tx => (
                    <div key={tx.id} className="flex justify-between py-1.5 text-xs">
                      ...
                      <span className={`font-bold ${amountColor(tx.type)}`}>$ {tx.amount.toFixed(2)}</span>
                    </div>
                  ))
                )}
              </div>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-brand-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b flex justify-between items-center bg-[#F8F8F8] sticky top-0">
              <h3 className="font-bold text-lg">Add Transaction</h3>
              <button type="button" onClick={() => setIsAddModalOpen(false)} className="cursor-pointer"><X className="w-4 h-4" /></button>
            </div>
            <form onSubmit={handleAddSubmit} className="p-6 space-y-4">
                  ...
                  <button
                    ...
                    className={`py-2 rounded-xl text-xs font-bold capitalize border-2 cursor-pointer ${
                      newTx.type === t
                        ...
                        : 'border-transparent bg-[#F8F8F8] text-brand-black/50'
                    }`}
                  >
                    {t}
                  </button>
              ...
              <input
                ...
                placeholder="Amount"
                className="w-full bg-[#F8F8F8] rounded-xl px-4 py-3 text-lg font-bold outline-none"
              />
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-[#F8F8F8] rounded-xl px-3 py-2.5 text-sm font-semibold cursor-pointer"
                    >
                  ...
                  <div>
                    ...
                    <select
                      value={newTx.targetAccountId}
                      onChange={e => setNewTx(p => ({ ...p, targetAccountId: e.target.value }))}
                      className="w-full bg-[#F8F8F8] rounded-xl px-3 py-2.5 text-sm font-semibold cursor-pointer"
                    >
                      {accounts.map(a => <option key={a.id} value={a.id}>{a.name}</option>)}
                    </select>
              ...
              <input
                ...
                className="w-full bg-[#F8F8F8] rounded-xl px-4 py-2.5 text-sm font-semibold outline-none cursor-pointer"
              />
                    ...
                    {tags.map(tag => (
                      <button
                        ...
                        className={`px-3 py-1 rounded-full text-xs font-bold cursor-pointer ${
                          newTx.tagIds.includes(tag.id)
                            ? 'bg-brand-black text-brand-primary'
                            : 'bg-[#F8F8F8] text-brand-black/60'
                        }`}
                      >
                        {tag.name}
                      </button>
