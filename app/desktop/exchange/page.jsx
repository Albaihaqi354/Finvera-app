    ...
    <div className="flex flex-col h-[calc(100vh-140px)] space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button className="flex items-center gap-1.5 bg-brand-black hover:bg-brand-black/80 text-brand-primary px-4 py-2 rounded-xl text-xs font-bold transition-all shadow-sm">
          ...
          <div className="flex flex-col">
            {MOCK_EXCHANGE.map(ex => (
              <div key={ex.id} className="grid grid-cols-[80px_1fr_1fr_1fr_100px] gap-2 px-5 py-4 border-b border-brand-black/5 hover:bg-base-light transition-colors items-center group">
                ...
                <div>
                  <span className="text-sm font-bold text-brand-black">{ex.base} → {ex.target}</span>
                </div>
                <div>
                  <span className="text-sm font-bold text-brand-black/80">{ex.rate.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                </div>
                <div>
                  <span className="text-xs font-semibold text-brand-black/50">{ex.updated}</span>
                </div>
